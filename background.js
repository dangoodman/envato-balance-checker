...
function getCodecanyonUrl() {
    return "http://codecanyon.net/";
}

function getFeedUrl() {
    return getCodecanyonUrl();
}
...
function updateIcon() {
    if (!localStorage.hasOwnProperty('unreadCount')) {
        ...
        chrome.browserAction.setBadgeBackgroundColor({color: [65, 117, 0, 255]});
...
function getInboxCount(onSuccess, onError) {
    ...
    try {
        xhr.onreadystatechange = function () {
            ...
            if (xhr.response) {
                var matches = xhr.response.match(/header-logo-account__balance[^"]*[^>]*>([^<]+)/);
                if (matches && matches[1]) {
                    handleSuccess(Math.floor(parseFloat(matches[1].replace(/[^\d]/, ''))));
                    return;
                } else {
...
function goToWebsite() {
    chrome.tabs.create({url: getCodecanyonUrl()});
}
...
chrome.browserAction.onClicked.addListener(function() {
    startRequest({showLoadingAnimation: true});
    goToWebsite();
});
